...
struct Coef get_shelving_coef(smp_type dbGain, smp_type fc, smp_type fs, smp_type Q)
{
	...
	shelving_coef.b0 = (V0 + root2 * sqrt(V0) * K + K_squre) / (1 + root2 * K + K_squre);
	shelving_coef.b1 = (2 * (K_squre - V0) ) / (1 + root2 * K + K_squre);
	shelving_coef.b2 = (V0 - root2 * sqrt(V0)*K + K_squre) / (1 + root2 * K + K_squre);
	shelving_coef.a1 = (2 * (K_squre - 1) ) / (1 + root2 * K + K_squre);
	shelving_coef.a2 = (1 - root2 * K + K_squre) / (1 + root2 * K + K_squre);

	return shelving_coef;
}
...
void pre_emphasis(smp_type data[], int arr_length, struct Coef shelving_coef)
{
	...
	for (index = 0; index < arr_length; index++) {
		/* compute output */
		yBuffer[current] = shelving_coef.b0 * data[index] + shelving_coef.b1 * xBuffer[INDEX(current-1)] + shelving_coef.b2 * xBuffer[INDEX(current-2)] - shelving_coef.a1 * yBuffer[INDEX(current-1)] - shelving_coef.a2 * yBuffer[INDEX(current-2)];
...
int main() {
	...
	struct Coef shelving_coef = get_shelving_coef(6, 1000, 16E3, 0.9);
	pre_emphasis(test_input, window_length, shelving_coef);
