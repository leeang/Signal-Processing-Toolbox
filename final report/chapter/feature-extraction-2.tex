...
$f[m]$ ($m = 0, 1, \dots, M+1$) are equally spaced in the mel-scale. We take $f_{\min}$ = 0 Hz and $f_{\max} = \frac{F_s}{2}$ = 8000 Hz as per the system specification and $M = 20$ according to \cite{davis1980comparison}. $f[m]$ ($m = 0, 1, \dots, M+1$) are computed by (\ref{eq:bank-boundary}) and listed in the appendix on page \pageref{table:frequency-mel-relationship}.
\begin{equation}
\label{eq:bank-boundary}
f[m] = \mel^{-1} \left( \mel(f_{\min}) + m \cdot \frac{\mel(f_{\max}) - \mel(f_{\min})}{M + 1} \right)
\end{equation}
where the mel-scale to frequency transform $\mel^{-1}(m_{mel})$ is the inverse function of (\ref{eq:mel-function}) on page \pageref{eq:mel-function}.
\begin{equation}
