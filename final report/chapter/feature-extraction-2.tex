...
(\ref{eq:power-spectrum}) can be further simplified because $H_{mel}[m, 0]$ = $H_{mel}[m, \frac{N}{2}+1] = 0$, $\forall m = 1, 2, \dots, M$. Fig. \ref{mel_triangle} also visualizes this property. Hence, only the first half of the \textbf{complex} spectrum need to be taken into consideration
\begin{equation}
