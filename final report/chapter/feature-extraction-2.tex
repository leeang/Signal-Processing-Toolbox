...
\centering
\includegraphics[width=\textwidth]{ang/pre_emphasis_filter}
...
Suggested by Professor Erik \textsc{Weyer}, we try to devise a \textit{shelving filter} represented by (\ref{shelving-filter}) to pre-emphasize the speech signal. By trial and error, we eventually manage to obtain a appropriate filter that amplifies high frequency components without attenuating frequencies lower than 1 kHz (shown in Fig. \ref{pre_emphasis_filter} by \textcolor{navy_matlab}{navy} solid line). Fig. \ref{zero_fft} shows the spectra of word \textit{zero} before and after this pre-emphasis filter.
...
\hat{X}_j[m] = \ln(X_j[m]) \quad m = 1, 2, \dots, M
\end{equation}
