...
void pre_emphasis(fract16 data[], int arr_length, struct Coef shelving_coef)
{
	...
	for (index = 0; index < arr_length; index++) {
		...
		yBuffer[current] = sub_fr1x32(temp_b, temp_a) << 2;
...
int main() {
	...
	fract16 hamming_coef[WINDOW_LENGTH];
	get_hamming_coef(hamming_coef, WINDOW_LENGTH);
	hamming(data, WINDOW_LENGTH, hamming_coef);
